  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      if (modeStatus.default) {
        ...
        response = await axios.post(`${URL}/validateCode`, { email: savedEmail, code: confirmNum });
        localStorage.setItem('testEmail1', email);
        ...
        <div className="container flex">
          ...
          <form onSubmit={handleSubmit} className='flex'>
            ...
            <div className='div1 flex'>
              ...
              <p>Use your email or another service to access {COMPANY}.</p>
            </div>

            <span className={message ? "showMessage" : "hiddenMessage"}>{message}</span>

            {modeStatus.default && (
              <div className='div1 flex'>
                <GoogleLogin onSuccess={jwt => { handleLoginGoogle(jwt) }} onError={() => { console.log('Login Failed'); }} />
              </div>
            )}
