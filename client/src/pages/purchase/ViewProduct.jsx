...
function ViewProduct() {
  ...
  const verifyUser = (isBuyNow) => {
    if (sellerId === userId) {
      ...
      localStorage.setItem("buyNow", JSON.stringify({ id: id, name: product.name, price: product.price, quantity: 1 }))
        ...
        <div className='productInfo flex'>
          ...
          {showQuantity &&
            <div className='showQuantity'>
              ...
              <p>How many units would you like to {isBuyNow ? "buy" : "add to cart"}?</p>
          ...
          <p>Sold by <b>{sellerName}</b> <b>{dateToDay(product.createdAt)}</b> days ago</p>

          <p><b>{product.quantity}</b> units left</p>
